...
RUN_FOLDER = PROJECT_DIR / "results" / "run_20260730_105710"
...
""" ### Tabela dos resultados (organizada)"""
# Ordem desejada das estratégias
strategy_order = [
    "NoQuery",
    "FullSupervision",
    "Random",
...
df_results_online["Strategy"] = pd.Categorical(
    df_results_online["Strategy"],
    categories=strategy_order,
    ordered=True
)

df_results_pool["Strategy"] = pd.Categorical(
    df_results_pool["Strategy"],
    categories=strategy_order,
    ordered=True
)
...
df_results_online = (
    df_results_online
    .sort_values(["Dataset", "Strategy", "Budget"], na_position="first")
...
df_results_online.to_csv(
    RUN_FOLDER / "online_results_sorted.csv",
    index=False
)

df_results_pool.to_csv(
    RUN_FOLDER / "pool_results_sorted.csv",
    index=False
)
